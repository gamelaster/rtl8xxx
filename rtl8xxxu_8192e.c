...
rtl8192e_set_tx_power(struct rtl8xxxu_priv *priv, int channel, bool ht40)
{
	...
	priv->iops->write32(priv, REG_TX_AGC_A_MCS11_MCS08, mcs);
	priv->iops->write32(priv, REG_TX_AGC_A_MCS15_MCS12, mcs);
	...
	if (priv->tx_paths > 1) {
		...
		priv->iops->write32(priv, REG_TX_AGC_B_RATE18_06, ofdm);
		priv->iops->write32(priv, REG_TX_AGC_B_RATE54_24, ofdm);
		...
		priv->iops->write32(priv, REG_TX_AGC_B_MCS03_MCS00, mcs);
		priv->iops->write32(priv, REG_TX_AGC_B_MCS07_MCS04, mcs);
		priv->iops->write32(priv, REG_TX_AGC_B_MCS11_MCS08, mcs);
		priv->iops->write32(priv, REG_TX_AGC_B_MCS15_MCS12, mcs);
	}
}
...
static int rtl8192eu_iqk_path_a(struct rtl8xxxu_priv *priv)
{
	 ...
	 */
	priv->iops->write32(priv, REG_FPGA0_IQK, 0x00000000);
	...
	priv->iops->write32(priv, REG_TX_IQK_PI_A, 0x82140303);
	priv->iops->write32(priv, REG_RX_IQK_PI_A, 0x68160000);
...
static int rtl8192eu_rx_iqk_path_a(struct rtl8xxxu_priv *priv)
{
	...
	/* Leave IQK mode */
	priv->iops->write32(priv, REG_FPGA0_IQK, 0x00);
	...
	priv->iops->write32(priv, REG_TX_IQK_PI_A, 0x82160c1f);
	priv->iops->write32(priv, REG_RX_IQK_PI_A, 0x68160c1f);

	/* LO calibration setting */
	priv->iops->write32(priv, REG_IQK_AGC_RSP, 0x0046a911);

	/* One shot, path A LOK & IQK */
	priv->iops->write32(priv, REG_IQK_AGC_PTS, 0xfa000000);
	priv->iops->write32(priv, REG_IQK_AGC_PTS, 0xf8000000);
	...
	priv->iops->write32(priv, REG_TX_IQK_PI_A, 0x82160c1f);
	priv->iops->write32(priv, REG_RX_IQK_PI_A, 0x28160c1f);

	/* LO calibration setting */
	priv->iops->write32(priv, REG_IQK_AGC_RSP, 0x0046a891);
...
static int rtl8192eu_iqk_path_b(struct rtl8xxxu_priv *priv)
{
	...
	priv->iops->write32(priv, REG_TX_IQK_TONE_A, 0x38008c1c);
	priv->iops->write32(priv, REG_RX_IQK_TONE_A, 0x38008c1c);
	priv->iops->write32(priv, REG_TX_IQK_TONE_B, 0x18008c1c);
	priv->iops->write32(priv, REG_RX_IQK_TONE_B, 0x38008c1c);

	priv->iops->write32(priv, REG_TX_IQK_PI_B, 0x821403e2);
	priv->iops->write32(priv, REG_RX_IQK_PI_B, 0x68160000);

	/* LO calibration setting */
	priv->iops->write32(priv, REG_IQK_AGC_RSP, 0x00492911);
...
static int rtl8192eu_rx_iqk_path_b(struct rtl8xxxu_priv *priv)
{
	...
	priv->iops->write32(priv, REG_TX_IQK_PI_B, 0x82160c1f);
	priv->iops->write32(priv, REG_RX_IQK_PI_B, 0x68160c1f);
	...
	priv->iops->write32(priv, REG_RX_IQK_TONE_A, 0x38008c1c);
	priv->iops->write32(priv, REG_TX_IQK_TONE_B, 0x38008c1c);
	priv->iops->write32(priv, REG_RX_IQK_TONE_B, 0x18008c1c);
				      ...
				      int result[][8], int t)
{
	...
	priv->iops->write32(priv, REG_FPGA0_XCD_RF_SW_CTRL, 0x22208200);
...
static void rtl8192e_crystal_afe_adjust(struct rtl8xxxu_priv *priv)
{
	...
	val32 = priv->iops->read32(priv, REG_AFE_CTRL4);
	val32 &= 0xfffffc7f;
	priv->iops->write32(priv, REG_AFE_CTRL4, val32);
...
static int rtl8192eu_active_to_lps(struct rtl8xxxu_priv *priv)
{
	 ...
	 */
	do {
		val32 = priv->iops->read32(priv, REG_SCH_TX_CMD);
		if (!val32) {
			retval = 0;
			break;
		}
...
static int rtl8192eu_active_to_emu(struct rtl8xxxu_priv *priv)
{
	...
	/* Turn off RF */
	val8 = priv->iops->read8(priv, REG_RF_CTRL);
	val8 &= ~RF_ENABLE;
	priv->iops->write8(priv, REG_RF_CTRL, val8);
...
static int rtl8192eu_power_on(struct rtl8xxxu_priv *priv)
{
	...
	if (val32 & SYS_CFG_SPS_LDO_SEL) {
		priv->iops->write8(priv, REG_LDO_SW_CTRL, 0xc3);
	} else {
		/*
		 * Raise 1.2V voltage
		 */
		val32 = priv->iops->read32(priv, REG_8192E_LDOV12_CTRL);
		val32 &= 0xff0fffff;
		val32 |= 0x00500000;
		priv->iops->write32(priv, REG_8192E_LDOV12_CTRL, val32);
		priv->iops->write8(priv, REG_LDO_SW_CTRL, 0x83);
	}
	...
	priv->iops->write16(priv, REG_CR, 0x0000);
