...
rtl8723b_set_tx_power(struct rtl8xxxu_priv *priv, int channel, bool ht40)
{
	...
	priv->iops->write32(priv, REG_TX_AGC_A_RATE18_06, ofdm);
	priv->iops->write32(priv, REG_TX_AGC_A_RATE54_24, ofdm);
	...
	priv->iops->write32(priv, REG_TX_AGC_A_MCS03_MCS00, mcs);
	priv->iops->write32(priv, REG_TX_AGC_A_MCS07_MCS04, mcs);
}
...
static void rtl8723bu_init_phy_bb(struct rtl8xxxu_priv *priv)
{
	...
	priv->iops->write32(priv, REG_S0S1_PATH_SWITCH, 0x00);
	...
	/* Why? */
	priv->iops->write8(priv, REG_SYS_FUNC, 0xe3);
	priv->iops->write8(priv, REG_AFE_XTAL_CTRL + 1, 0x80);
...
static void rtl8723bu_phy_init_antenna_selection(struct rtl8xxxu_priv *priv)
{
	...
	val32 = priv->iops->read32(priv, REG_PAD_CTRL1);
	val32 &= ~(BIT(20) | BIT(24));
	priv->iops->write32(priv, REG_PAD_CTRL1, val32);

	val32 = priv->iops->read32(priv, REG_GPIO_MUXCFG);
	val32 &= ~BIT(4);
	priv->iops->write32(priv, REG_GPIO_MUXCFG, val32);

	val32 = priv->iops->read32(priv, REG_GPIO_MUXCFG);
	val32 |= BIT(3);
	priv->iops->write32(priv, REG_GPIO_MUXCFG, val32);

	val32 = priv->iops->read32(priv, REG_LEDCFG0);
	val32 |= BIT(24);
	priv->iops->write32(priv, REG_LEDCFG0, val32);

	val32 = priv->iops->read32(priv, REG_LEDCFG0);
	val32 &= ~BIT(23);
	priv->iops->write32(priv, REG_LEDCFG0, val32);

	val32 = priv->iops->read32(priv, REG_RFE_BUFFER);
	val32 |= (BIT(0) | BIT(1));
	priv->iops->write32(priv, REG_RFE_BUFFER, val32);

	val32 = priv->iops->read32(priv, REG_RFE_CTRL_ANTA_SRC);
	val32 &= 0xffffff00;
	val32 |= 0x77;
	priv->iops->write32(priv, REG_RFE_CTRL_ANTA_SRC, val32);

	val32 = priv->iops->read32(priv, REG_PWR_DATA);
	val32 |= PWR_DATA_EEPRPAD_RFE_CTRL_EN;
	priv->iops->write32(priv, REG_PWR_DATA, val32);
}
...
static int rtl8723bu_iqk_path_a(struct rtl8xxxu_priv *priv)
{
	...
	path_sel = priv->iops->read32(priv, REG_S0S1_PATH_SWITCH);

	/*
	 * Leave IQK mode
	 */
	val32 = priv->iops->read32(priv, REG_FPGA0_IQK);
	val32 &= 0x000000ff;
	priv->iops->write32(priv, REG_FPGA0_IQK, val32);
	...
	/* path-A IQK setting */
	priv->iops->write32(priv, REG_TX_IQK_TONE_A, 0x18008c1c);
	priv->iops->write32(priv, REG_RX_IQK_TONE_A, 0x38008c1c);
	priv->iops->write32(priv, REG_TX_IQK_TONE_B, 0x38008c1c);
	priv->iops->write32(priv, REG_RX_IQK_TONE_B, 0x38008c1c);

	priv->iops->write32(priv, REG_TX_IQK_PI_A, 0x821403ea);
	priv->iops->write32(priv, REG_RX_IQK_PI_A, 0x28110000);
	priv->iops->write32(priv, REG_TX_IQK_PI_B, 0x82110000);
	priv->iops->write32(priv, REG_RX_IQK_PI_B, 0x28110000);

	/* LO calibration setting */
	priv->iops->write32(priv, REG_IQK_AGC_RSP, 0x00462911);
	 ...
	 */
	if (priv->rf_paths > 1)
		priv->iops->write32(priv, REG_S0S1_PATH_SWITCH, 0x00000000);
	else
		priv->iops->write32(priv, REG_S0S1_PATH_SWITCH, 0x00000280);

	/*
	 * Bit 12 seems to be BT_GRANT, and is only found in the 8723bu.
	 * No trace of this in the 8192eu or 8188eu vendor drivers.
	 */
	priv->iops->write32(priv, REG_BT_CONTROL_8723BU, 0x00000800);
	...
	/* Restore Ant Path */
	priv->iops->write32(priv, REG_S0S1_PATH_SWITCH, path_sel);
#ifdef RTL8723BU_BT
	/* GNT_BT = 1 */
	priv->iops->write32(priv, REG_BT_CONTROL_8723BU, 0x00001800);
#endif
...
static int rtl8723bu_rx_iqk_path_a(struct rtl8xxxu_priv *priv)
{
	...
	priv->iops->write32(priv, REG_TX_IQK_PI_A, 0x82160ff0);
	priv->iops->write32(priv, REG_RX_IQK_PI_A, 0x28110000);
	priv->iops->write32(priv, REG_TX_IQK_PI_B, 0x82110000);
	priv->iops->write32(priv, REG_RX_IQK_PI_B, 0x28110000);

	/* LO calibration setting */
	priv->iops->write32(priv, REG_IQK_AGC_RSP, 0x0046a911);
	...
	priv->iops->write32(priv, REG_TX_IQK, val32);
	...
	/* path-A IQK setting */
	priv->iops->write32(priv, REG_TX_IQK_TONE_A, 0x38008c1c);
	priv->iops->write32(priv, REG_RX_IQK_TONE_A, 0x18008c1c);
	priv->iops->write32(priv, REG_TX_IQK_TONE_B, 0x38008c1c);
	priv->iops->write32(priv, REG_RX_IQK_TONE_B, 0x38008c1c);

	priv->iops->write32(priv, REG_TX_IQK_PI_A, 0x82110000);
	priv->iops->write32(priv, REG_RX_IQK_PI_A, 0x2816001f);
	priv->iops->write32(priv, REG_TX_IQK_PI_B, 0x82110000);
	priv->iops->write32(priv, REG_RX_IQK_PI_B, 0x28110000);

	/* LO calibration setting */
	priv->iops->write32(priv, REG_IQK_AGC_RSP, 0x0046a8d1);
				      ...
				      int result[][8], int t)
{
	...
	u8 xa_agc = priv->iops->read32(priv, REG_OFDM0_XA_AGC_CORE1) & 0xff;
	u8 xb_agc = priv->iops->read32(priv, REG_OFDM0_XB_AGC_CORE1) & 0xff;
	...
	if (t) {
		...
		val32 = priv->iops->read32(priv, REG_OFDM0_XA_AGC_CORE1);
		val32 &= 0xffffff00;
		priv->iops->write32(priv, REG_OFDM0_XA_AGC_CORE1, val32 | 0x50);
		priv->iops->write32(priv, REG_OFDM0_XA_AGC_CORE1, val32 | xa_agc);

		if (priv->tx_paths > 1) {
			val32 = priv->iops->read32(priv, REG_OFDM0_XB_AGC_CORE1);
			val32 &= 0xffffff00;
			priv->iops->write32(priv, REG_OFDM0_XB_AGC_CORE1,
					 val32 | 0x50);
			priv->iops->write32(priv, REG_OFDM0_XB_AGC_CORE1,
					 val32 | xb_agc);
		}
...
static void rtl8723bu_phy_iq_calibrate(struct rtl8xxxu_priv *priv)
{
	...
	bt_control = priv->iops->read32(priv, REG_BT_CONTROL_8723BU);
	...
	priv->iops->write32(priv, REG_BT_CONTROL_8723BU, bt_control);
...
static int rtl8723bu_active_to_emu(struct rtl8xxxu_priv *priv)
{
	...
	val16 = priv->iops->read16(priv, REG_GPIO_INTM);
	val16 &= ~GPIO_INTM_EDGE_TRIG_IRQ;
	priv->iops->write16(priv, REG_GPIO_INTM, val16);
	...
	val8 = priv->iops->read8(priv, REG_AFE_MISC);
	val8 &= ~AFE_MISC_WL_XTAL_CTRL;
	priv->iops->write8(priv, REG_AFE_MISC, val8);
...
static int rtl8723b_emu_to_active(struct rtl8xxxu_priv *priv)
{
	...
	val32 = priv->iops->read8(priv, REG_APS_FSMCO);
	...
	val8 = priv->iops->read8(priv, REG_GPIO_INTM + 1);
	val8 |= BIT(1);
	priv->iops->write8(priv, REG_GPIO_INTM + 1, val8);

	/* Enable GPIO9 interrupt mode */
	val8 = priv->iops->read8(priv, REG_GPIO_IO_SEL_2 + 1);
	val8 |= BIT(1);
	priv->iops->write8(priv, REG_GPIO_IO_SEL_2 + 1, val8);

	/* Enable GPIO9 input mode */
	val8 = priv->iops->read8(priv, REG_GPIO_IO_SEL_2);
	val8 &= ~BIT(1);
	priv->iops->write8(priv, REG_GPIO_IO_SEL_2, val8);

	/* Enable HSISR GPIO[C:0] interrupt */
	val8 = priv->iops->read8(priv, REG_HSIMR);
	val8 |= BIT(0);
	priv->iops->write8(priv, REG_HSIMR, val8);

	/* Enable HSISR GPIO9 interrupt */
	val8 = priv->iops->read8(priv, REG_HSIMR + 2);
	val8 |= BIT(1);
	priv->iops->write8(priv, REG_HSIMR + 2, val8);

	val8 = priv->iops->read8(priv, REG_MULTI_FUNC_CTRL);
	val8 |= MULTI_WIFI_HW_ROF_EN;
	priv->iops->write8(priv, REG_MULTI_FUNC_CTRL, val8);

	/* For GPIO9 internal pull high setting BIT(14) */
	val8 = priv->iops->read8(priv, REG_MULTI_FUNC_CTRL + 1);
	val8 |= BIT(6);
	priv->iops->write8(priv, REG_MULTI_FUNC_CTRL + 1, val8);
...
static int rtl8723bu_power_on(struct rtl8xxxu_priv *priv)
{
	 ...
	 * antenna parts.
	 */
	priv->iops->write8(priv, REG_PAD_CTRL1 + 3, 0x20);

	val16 = priv->iops->read16(priv, REG_SYS_FUNC);
	val16 |= SYS_FUNC_BBRSTB | SYS_FUNC_BB_GLB_RSTN;
	priv->iops->write16(priv, REG_SYS_FUNC, val16);

	priv->iops->write8(priv, REG_BT_CONTROL_8723BU + 1, 0x18);
	priv->iops->write8(priv, REG_WLAN_ACT_CONTROL_8723B, 0x04);
	priv->iops->write32(priv, REG_S0S1_PATH_SWITCH, 0x00);
	/* Antenna inverse */
	priv->iops->write8(priv, 0xfe08, 0x01);

	val16 = priv->iops->read16(priv, REG_PWR_DATA);
	val16 |= PWR_DATA_EEPRPAD_RFE_CTRL_EN;
	priv->iops->write16(priv, REG_PWR_DATA, val16);

	val32 = priv->iops->read32(priv, REG_LEDCFG0);
	val32 |= LEDCFG0_DPDT_SELECT;
	priv->iops->write32(priv, REG_LEDCFG0, val32);

	val8 = priv->iops->read8(priv, REG_PAD_CTRL1);
	val8 &= ~PAD_CTRL1_SW_DPDT_SEL_DATA;
	priv->iops->write8(priv, REG_PAD_CTRL1, val8);
exit:
	return ret;
}
...
static void rtl8723bu_power_off(struct rtl8xxxu_priv *priv)
{
	...
	priv->iops->write8(priv, REG_CR, 0x0000);
...
static void rtl8723b_enable_rf(struct rtl8xxxu_priv *priv)
{
	 ...
	 */
	priv->iops->write8(priv, 0x0790, 0x05);
	/*
	 ...
	 */
	priv->iops->write8(priv, 0x0778, 0x01);

	val8 = priv->iops->read8(priv, REG_GPIO_MUXCFG);
	val8 |= BIT(5);
	priv->iops->write8(priv, REG_GPIO_MUXCFG, val8);
	 ...
	 */
	priv->iops->write8(priv, 0x0974, 0xff);

	val32 = priv->iops->read32(priv, REG_RFE_BUFFER);
	val32 |= (BIT(0) | BIT(1));
	priv->iops->write32(priv, REG_RFE_BUFFER, val32);

	priv->iops->write8(priv, REG_RFE_CTRL_ANTA_SRC, 0x77);
	...
	priv->iops->write32(priv, REG_BT_COEX_TABLE1, 0x55555555);
	priv->iops->write32(priv, REG_BT_COEX_TABLE2, 0x55555555);
	priv->iops->write32(priv, REG_BT_COEX_TABLE3, 0x00ffffff);
	priv->iops->write8(priv, REG_BT_COEX_TABLE4, 0x03);
...
static void rtl8723bu_init_aggregation(struct rtl8xxxu_priv *priv)
{
	...
	agg_rx = priv->iops->read32(priv, REG_RXDMA_AGG_PG_TH);
	agg_rx &= ~RXDMA_USB_AGG_ENABLE;
	agg_rx &= ~0xff0f;

	priv->iops->write8(priv, REG_TRXDMA_CTRL, agg_ctrl);
	priv->iops->write32(priv, REG_RXDMA_AGG_PG_TH, agg_rx);
}
...
static void rtl8723bu_init_statistics(struct rtl8xxxu_priv *priv)
{
	...
	priv->iops->write16(priv, REG_NHM_TIMER_8723B + 2, 0x2710);
	priv->iops->write16(priv, REG_NHM_TH9_TH10_8723B + 2, 0xffff);
	priv->iops->write32(priv, REG_NHM_TH3_TO_TH0_8723B, 0xffffff52);
	priv->iops->write32(priv, REG_NHM_TH7_TO_TH4_8723B, 0xffffffff);
	/* TH8 */
	val32 = priv->iops->read32(priv, REG_FPGA0_IQK);
	val32 |= 0xff;
	priv->iops->write32(priv, REG_FPGA0_IQK, val32);
	/* Enable CCK */
	val32 = priv->iops->read32(priv, REG_NHM_TH9_TH10_8723B);
	val32 |= BIT(8) | BIT(9) | BIT(10);
	priv->iops->write32(priv, REG_NHM_TH9_TH10_8723B, val32);
	/* Max power amongst all RX antennas */
	val32 = priv->iops->read32(priv, REG_OFDM0_FA_RSTC);
	val32 |= BIT(7);
	priv->iops->write32(priv, REG_OFDM0_FA_RSTC, val32);
}
