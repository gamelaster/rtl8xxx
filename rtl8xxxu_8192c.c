...
static int rtl8192cu_power_on(struct rtl8xxxu_priv *priv)
{
	...
	for (i = 100; i; i--) {
		val8 = priv->iops->read8(priv, REG_APS_FSMCO);
	...
	priv->iops->write8(priv, REG_SPS0_CTRL, 0x2b);
	udelay(100);

	val8 = priv->iops->read8(priv, REG_LDOV12D_CTRL);
	if (!(val8 & LDOV12D_ENABLE)) {
		pr_info("%s: Enabling LDOV12D (%02x)\n", __func__, val8);
		val8 |= LDOV12D_ENABLE;
		priv->iops->write8(priv, REG_LDOV12D_CTRL, val8);
	 ...
	 * Auto enable WLAN
	 */
	val16 = priv->iops->read16(priv, REG_APS_FSMCO);
	val16 |= APS_FSMCO_MAC_ENABLE;
	priv->iops->write16(priv, REG_APS_FSMCO, val16);
	...
	val8 = priv->iops->read8(priv, REG_APSD_CTRL);
	val8 &= ~APSD_CTRL_OFF;
	priv->iops->write8(priv, REG_APSD_CTRL, val8);
	for (i = 200; i; i--) {
		val8 = priv->iops->read8(priv, REG_APSD_CTRL);
	...
	priv->iops->write8(priv, 0xfe10, 0x19);
